...
#define FISH_WEBSITE_FROM "http.p2hp.com" // 钓鱼网站源网址
...
// 接受来自服务器的数据
bool ReceiveData(SOCKET socket, char *buffer, int &recvLen)
{
...
DWORD __stdcall ProxyThread(LPVOID lpParameter)
{
    ...
    // 接收来自客户端的数据
    if (!ReceiveData(clientSocket, buffer, recvLen))
    {
    ...
    // 连接目标服务器
    printf("\n=====================================\n");
    printf("当前连接的服务器host为: %s", httpHeader.host);
    printf("\n=====================================\n");
    ...
    if (ReceiveData(serverSocket, buffer, recvLen) && recvLen != 0)
    {

        if (IsCacheExist(cacheFileName))
        {
            printf("host:%s 缓存文件存在\n", httpHeader.host);
