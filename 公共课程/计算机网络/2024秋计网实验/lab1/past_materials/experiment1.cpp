...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	// 系统通过句柄来引用和操作线程对象。句柄是一个32位的无符号整数值，用来标识一个线程对象。
	HANDLE hThread;
	DWORD dwThreadID; // unsigned long 没用上
	...
	while (true)
	{
		...
		// LPVOID表示一个指向任意类型的指针。LP 是 "Long Pointer" 的缩写，VOID 表示没有具体类型。LPVOID 可以用来指向任何类型的数据。
...
unsigned int __stdcall ProxyThread(LPVOID lpParameter)
{
	// 定义缓存变量
	char filename[100] = {0};
	FILE *_Post_ in; // 定义了一个文件指针，用于指向输入文件。_Post_ _Notnull_是一个宏，表示在函数返回之后，in指针必须不为空
	// datebuffer也米用上
	...
	SOCKADDR_IN clientAddr; // 这个变量没有使用
	...
	printf("发送给目标服务器的数据 : %s\n ========================================= \n", Buffer);
	ret = send(((ProxyParam *)lpParameter)->serverSocket, Buffer, strlen(Buffer) + 1, 0);

	// 等待目标服务器返回数据
	recvSize = recv(((ProxyParam *)lpParameter)->serverSocket, Buffer, MAXSIZE, 0);
	printf("接收到的目标服务器的数据 : %s\n ========================================= \n", Buffer);
	if (recvSize <= 0)
	{
		goto error;
	}
...
// 根据url构造文件名，加入 cache 子目录
void makeFilename(char *url, char *filename)
{
	strcpy_s(filename, 100, "cache\\"); // 添加 cache 目录
	while (*url != '\0')
	{
		if ('a' <= *url && *url <= 'z')
		{
